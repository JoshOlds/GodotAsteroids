...
## Reference to AsteroidCleaner used to track all active asteroids. Newly generated asteroids will be added to AsteroidCleaner's array of Asteroids
...
func generate_asteroid(radius: int, vertice_count: int, jaggedness : float, spawn_position: Vector2, force_vector : Vector2, angular_velocity : float):
	...
	asteroid.setup(radius, asteroid_mass_multiplier, asteroid_health_multiplier, vertice_count, jaggedness, asteroid_cleaner_ref)
...
## Checks supplied position and radius against all existing asteroids in the AsteroidCleaner and returns whether the supplied position is free of overlap
func check_position_is_clear_of_asteroids(radius : int, position : Vector2) -> bool:
	var position_clear = true
	for asteroid in asteroid_cleaner_ref.asteroids:
