...
## The asteroid cleaner to add this asteroid to on spawn
...
var proc_asteroid = preload("res://scenes/asteroids/proc_asteroid.tscn") # proc_asteroid.tscn


## Sets up this object. Must be called after instantiating scene, before adding to scene tree
func setup(_radius: float, _mass_multiplier : float, _health_multiplier : float, _vertice_count: int, _jaggedness: float, _asteroid_cleaner_ref : AsteroidCleaner):
	radius = _radius
	...
	asteroid_cleaner_ref = _asteroid_cleaner_ref
...
func _ready():
	...
	## Add this asteroid to the cleaner. Used for tracking deletion and efficient querying of all asteroids
	asteroid_cleaner_ref.add_asteroid(self)
...
func on_health_expired(damage_source_node : Node):
	if radius >= min_split_radius:
		split_asteroid()
	asteroid_cleaner_ref.asteroids.erase(self) # TODO: Erasing should defer until later time - don't erase from array
...
func split_asteroid():
	...
	asteroid_cleaner_ref.call_deferred("add_child", child_asteroid_1)
	asteroid_cleaner_ref.call_deferred("add_child", child_asteroid_2)
...
func generate_child_asteroid(input_radius : float = 0) -> ProcAsteroid:
	...
	child.setup(child_radius, mass_multiplier, health_multiplier, child_vertice_count, jaggedness, asteroid_cleaner_ref)
	return child
